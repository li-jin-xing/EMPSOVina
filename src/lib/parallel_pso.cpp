...
struct parallel_pso_aux {
	...
	parallel_pso_aux(const pso_search* pso_, const precalculate* p_, const igrid* ig_, const precalculate* p_widened_, const igrid* ig_widened_, const vec* corner1_, const vec* corner2_, parallel_progress* pg_, int num_birds_, int struggle_, double w_, double c1_, double c2_)
		: pso(pso_), p(p_), ig(ig_), p_widened(p_widened_), ig_widened(ig_widened_), corner1(corner1_), corner2(corner2_), pg(pg_), num_birds(num_birds_), struggle(struggle_), w(w_), c1(c1_), c2(c2_) {}
	void operator()(parallel_pso_task& t) const {
		(*pso)(t.m, t.out, *p, *ig, *p_widened, *ig_widened, *corner1, *corner2, pg, t.generator, num_birds, struggle, w, c1, c2);
	}
};
...
void parallel_pso::operator()(const model& m, output_container& out, const precalculate& p, const igrid& ig, const precalculate& p_widened, const igrid& ig_widened, const vec& corner1, const vec& corner2, rng& generator, int num_birds, int struggle, double w, double c1, double c2) const {
	parallel_progress pp;
	parallel_pso_aux parallel_pso_aux_instance(&pso, &p, &ig, &p_widened, &ig_widened, &corner1, &corner2, (display_progress ? (&pp) : NULL), num_birds, struggle, w, c1, c2);
