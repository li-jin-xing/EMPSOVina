...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights,int num_birds, int struggle, double w, double c1, double c2) {
	...
	if(score_only) {
		...
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_birds, struggle, w, c1, c2);
		done(verbosity, log);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, int num_birds,int struggle,double w,double c1,double c2) {
...
//定义和设置打分函数******************************************************************************************************
	...
	par.pso.num_saved_mins = num_modes;/*PSOVina*/
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights,num_birds,struggle,w,c1,c2);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "PSOVina version 2.0";    //pso
...
=================================================================================\n\
 ...
 For more information about AutoDock Vina, please visit http://vina.scripps.edu. \n\
                                                                                 \n\
 About more information of PSO Vina, please visit http://cbbio.cis.umac.mo.      \n\
                                                                                 \n\
=================================================================================\n";
	...
	try {
		...
		int cpu = 0, seed, exhaustiveness, verbosity = 2, num_modes = 9,num_birds = 0,struggle=0;
		fl energy_range = 2.0;
		...
		pso.add_options()
			("struggle", value<int>(&struggle)->default_value(30), "Struggling number of global search algorithm (Hybrid PSO)")
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, num_birds,struggle,w,c1,c2);
	}
	catch(file_error& e) {
